...
#################################################################################################
# You need this to succesfully create the exe file.
################################################################################################
...
class Canvas(QLabel):
    ...
    def initialize(self):
        ...
        self.mark_layer = QPixmap(self.width(), self.height())
        self.mark_layer.fill(Qt.transparent)
    ...
    def eraser_mouseMoveEvent(self, e):
        if self.last_pos:
            ...
            p2 = QPainter(self.mark_layer)
            p2.eraseRect(QRect(e.x(),e.y(),10,10))
    ...
    def pen_mouseMoveEvent(self, e):
        if self.last_pos:
            ...
            p2 = QPainter(self.mark_layer)
            p2.setPen(QPen(self.active_color, self.config['size'], Qt.SolidLine, Qt.SquareCap, Qt.RoundJoin))
            p2.drawLine(self.last_pos, e.pos())
    ...
    def brush_mouseMoveEvent(self, e):
        if self.last_pos:
            ...
            p2 = QPainter(self.mark_layer)
            p2.setPen(QPen(self.active_color, self.config['size'] * BRUSH_MULT, Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin))
            p2.drawLine(self.last_pos, e.pos())
    ...
    def spray_mouseMoveEvent(self, e):
        if self.last_pos:
            ...
            p2 = QPainter(self.mark_layer)
            p2.setPen(QPen(self.active_color, 1))
    ...
    def text_mousePressEvent(self, e):
        if e.button() == Qt.LeftButton and self.current_pos is None:
            ...
            p2.setFont(font)
            p2.setPen(pen)
            p2.drawText(self.current_pos, self.current_text)
            self.update()
    ...
    def text_timerEvent(self, final=False):
        ...
        p2 = QPainter(self.mark_layer)
        p2.setCompositionMode(QPainter.RasterOp_SourceXorDestination)
        p2.setPen(pen)
        ...
        if self.last_text:
            ...
            p2.setFont(font)
            p2.drawText(self.current_pos, self.last_text)
    ...
    def fill_mousePressEvent(self, e):
        ...
        p2 = QPainter(self.mark_layer)
        p2.setPen(QPen(self.active_color))

        while queue:
            x, y = queue.pop()
            if image.pixel(x, y) == target_color:
                p.drawPoint(QPoint(x, y))
                p2.drawPoint(QPoint(x, y))
    ...
    def generic_shape_timerEvent(self, final=False):
        ...
        if self.last_pos:
            ...
            getattr(p2, self.active_shape_fn)(QRect(self.origin_pos, self.last_pos), *self.active_shape_args)
        ...
        if not final:
            ...
            getattr(p2, self.active_shape_fn)(QRect(self.origin_pos, self.current_pos), *self.active_shape_args)
    ...
    def generic_shape_mouseReleaseEvent(self, e):
        if self.last_pos:
            ...
            p2 = QPainter(self.mark_layer)
            p2.setPen(QPen(self.primary_color, self.config['size'], Qt.SolidLine, Qt.SquareCap, Qt.MiterJoin))

            if self.config['fill']:
                p.setBrush(QBrush(self.secondary_color))
                p2.setBrush(QBrush(self.secondary_color))
            getattr(p, self.active_shape_fn)(QRect(self.origin_pos, e.pos()), *self.active_shape_args)
            getattr(p2, self.active_shape_fn)(QRect(self.origin_pos, e.pos()), *self.active_shape_args)
            self.update()
    ...
    def line_timerEvent(self, final=False):
        ...
        if self.last_pos:
            ...
            p2.drawLine(self.origin_pos, self.last_pos)

        if not final:
            p.drawLine(self.origin_pos, self.current_pos)
            p2.drawLine(self.origin_pos, self.current_pos)
    ...
    def line_mouseReleaseEvent(self, e):
        if self.last_pos:
            ...
            p2 = QPainter(self.mark_layer)
            p2.setPen(QPen(self.primary_color, self.config['size'], Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin))

            p2.drawLine(self.origin_pos, e.pos())
            self.update()
    ...
    def generic_poly_timerEvent(self, final=False):
        ...
        if self.last_history:
            ...
            getattr(p2, self.active_shape_fn)(*self.last_history)
        ...
        if not final:
            ...
            p2.setPen(pen)
            getattr(p2, self.active_shape_fn)(*self.history_pos + [self.current_pos])
    ...
    def generic_poly_mouseDoubleClickEvent(self, e):
        ...
        getattr(p2, self.active_shape_fn)(*self.history_pos + [e.pos()])
        self.update()
        self.reset_mode()
    ...
    def resizeEvent(self, event):  # QResizeEvent
        print("Size of canvas w=`{}`, h=`{}`".format(event.size().width(), event.size().height()))


        self.setScaledContents(True)
        # self.resize(event.size().width(), self.height())
        # self.reset()
        self.setPixmap(QPixmap( self.pixmap().scaled(self.width(), self.height(), Qt.IgnoreAspectRatio)))
        self.mark_layer = (QPixmap(self.mark_layer.scaled(self.width(), self.height(), Qt.IgnoreAspectRatio)))
        print("Label size {}".format(self.size()))
...
class MainWindow2(QMainWindow, Ui_MainWindow):
    ...
    def preset_image(self, fileName):
        fileName = fileName
        ...
        if fileName:
            ...
            self.canvas.setPixmap(QPixmap(pixmap.scaled(self.canvas.size(),Qt.IgnoreAspectRatio)))
    ...
    def save_file(self):
        ...
        :return:
        """
        path, _ = QFileDialog.getSaveFileName(self, "Save file", "Templates", "PNG Image file (*.png)")

        if path:
            pixmap = self.canvas.mark_layer
    ...
    #     # self.resize(event.size().width(), self.height())
    #     # self.reset()
    #     self.canvas.setPixmap(self.canvas.pixmap().scaled(self.canvas.width(), self.canvas.height(),Qt.KeepAspectRatio))
